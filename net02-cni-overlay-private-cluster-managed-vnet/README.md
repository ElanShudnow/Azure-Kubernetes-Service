...
## Private API Server with Managed VNET
...
When leveraging the --enable-private-cluster parameter during AKS Deployment without specifying custom Virtual Network settings or Private DNS Zone configurations (which we'll take a look at in another article), a managed Virtual Network is leveraged along with an AKS Managed Private DNS Zone, Private Endpoint, and Network Interface Card (NIC) assocated with the Private Endpoint. 
...
## Deployment
1. Download net02-cni-overlay-private-cluster-managed-vnet.azcli
...
3. Execute the commands in the script.
    ...
    ![Variables-01](../media/net02-cni-overlay-private-cluster-managed-vnet/variables01.png)

    ![Variables-02](../media/net02-cni-overlay-private-cluster-managed-vnet/variables02.png)

    #### Create Resource Group

    ![ResourceGroup-Creation](../media/net02-cni-overlay-private-cluster-managed-vnet/resourcegroup.png)

    #### Deploy AKS Cluster

    ![AKS Deployment](../media/net02-cni-overlay-private-cluster-managed-vnet/aksdeploy.png)
...
![JSON View 01](../media/net02-cni-overlay-private-cluster-managed-vnet/jsonview01.png)
...
![JSON View 02](../media/net02-cni-overlay-private-cluster-managed-vnet/jsonview02.png)
...
![Kubectl Get Nodes Failure](../media/net02-cni-overlay-private-cluster-managed-vnet/kubectlfailure.png)
...
For purposes of this lab, I am going to create another subnet in the AKS Managed VNET and deploy a Virtual Machine into this subnet called AKSPrivVM.  I will not demonstrate these steps as it is outside of the scope of this article.
...
![Kubectl Get Nodes](../media/net02-cni-overlay-private-cluster-managed-vnet/kubectlgetnodes.png)
...
![Quickstart Deploy](../media/net02-cni-overlay-private-cluster-managed-vnet/quickstartdeploy.png)
...
![Kubectl Get Service](../media/net02-cni-overlay-private-cluster-managed-vnet/kubectlgetservice.png)
...
![Kubectl Get Pod](../media/net02-cni-overlay-private-cluster-managed-vnet/kubectlgetpods.png)
...
As mentioned, in this lab, we've deployed our AKS Cluster using a managed Virtual Network and a managed Private DNS Zone and Private Endpoint.  Let's take a look under the hood as to how our connectivity to the Private API Server is working.
...
![JSON View 03](../media/net02-cni-overlay-private-cluster-managed-vnet/jsonview03.png)

The privateFQDN is: "<span style="color:green">aks-cluste-rg-aks-cluster-1df8be-swcreofl</span>.<span style="color:red">49e2c8cd-c252-4091-b804-093665977caf.privatelink.centralus.azmk8s.io</span>",

In our managed AKS Private DNS Zone, our Private DNS Zone is: <span style="color:red">49e2c8cd-c252-4091-b804-093665977caf.privatelink.centralus.azmk8s.io</span>.  Our A record for the Private Endpoint itself is <span style="color:green">aks-cluste-rg-aks-cluster-1df8be-swcreofl</span>.  I have color coded this for you to see how within the JSON View properties, the privateFQDN matches the Private DNS Zone and Private Endpoint together.

![JSON View 03](../media/net02-cni-overlay-private-cluster-managed-vnet/jsonview04.png)
...
![Kubectl Get Credentials](../media/net02-cni-overlay-private-cluster-managed-vnet/kubectlgetcreds.png)
...
![Kubectl Config privateFQDN](../media/net02-cni-overlay-private-cluster-managed-vnet/kubeconfig.png)
